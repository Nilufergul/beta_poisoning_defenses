...
# Farklı dataset boyutları
dataset_sizes = np.arange(100, 1500, 200)
f1_results = {"KPB": [], "NCC": [], "CBD": [], "MDT": []}

for size in dataset_sizes:
    # Veri setini ve model eğitimi
    tr, val, ts = load_mnist(digits=(4, 6), n_tr=size, n_val=size, n_ts=size)
    ...
    knn_ncc.fit(X_ncc)
    _, indices_ncc = knn_ncc.kneighbors(X_ncc)
    
    detected_poisoning_indices_ncc = []
    inner_neighbors_ncc = len(y_poison_all)
    outer_neighbors_ncc = inner_neighbors_ncc + int(inner_neighbors_ncc * 0.2)
    for i, neighbors in enumerate(indices_ncc):
        ...
        if np.bincount(inner).argmax() != np.bincount(outer).argmax():
...
for idx, method in enumerate(f1_results.keys()):
    plt.plot(dataset_sizes, f1_results[method], marker='o', linewidth=2,
             color=colors[idx], label=method)

plt.title("F1 Score vs Dataset Size for Beta Poisoning Defenses", fontsize=14)
plt.xlabel("Dataset Size", fontsize=14)
plt.ylabel("F1 Score", fontsize=14)
plt.ylim(0, 1)
plt.grid(True)
plt.legend(title="Defense Methods")
