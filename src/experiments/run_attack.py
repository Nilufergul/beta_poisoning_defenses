...
def run_attack(generator, path, clf, tr, val, ts, params):
    ...
    #val.X verisindeki X özelliklerinin minimum ve maksimum değerlerini içeren bir tuple’dır ve genellikle zehirlenmiş noktaları veya veri dağılımını belirli bir aralıkta tutmak için kullanılır.

    n_poisoning_points = np.linspace(start=0.20, stop=0.25, num=20) * tr.Y.size 
    g_name = path.split("/")[-1]
    all_poisoning_points = []

    with open(path + ".csv", "w") as file:
        writer = csv.writer(
            file, escapechar=" ", quoting=csv.QUOTE_NONE, delimiter=","
        )
        writer.writerow(
            [
        ...
        for _ in range(1):
            for c in [1]:
                ...
                for n_poison in n_poisoning_points.astype(int):
                    start = time.time()
                    x_poison, y_poison, x_proto = generator(
                        tr,
                        val,
                        clf,
                        box,
                    ...
                    all_poisoning_points.append((x_poison, y_poison))
                    write_bin(
                        res, tr, val, ts, clf, c, g_name, exec_time, params, writer,
                    )
                    file.flush()
        file.close()
    return all_poisoning_points, x_proto