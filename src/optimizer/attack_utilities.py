...
def pick_random_protos(ds, y, k=10):
    # y sınıfına ait verileri filtrele
    samples_in_y = ds.X[ds.Y == y, :]

    # Eğer hiç veri yoksa hata fırlat
    if samples_in_y.shape[0] == 0:
        raise ValueError(f"No data points found for class {y}. Cannot pick random prototypes.")
    
    # Eğer mevcut örnek sayısı k'dan küçükse, k'yı mevcut örnek sayısına ayarla
    if samples_in_y.shape[0] < k:
        k = samples_in_y.shape[0]

    # Verileri karıştır (shuffle)
    samples_in_y.shuffle()

    # İlk k örneği seç ve ndarray olarak döndür
