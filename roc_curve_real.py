...
tr, val, ts = load_mnist(digits=(0, 8), n_tr=300, n_val=300, n_ts=300)
...
class_label1 = 1  # Pozitif sınıf (zehirlenmiş sınıfı) 1 yapıyoruz
class_indices1 = np.where(all_Y == class_label1)[0]
class_points1 = all_X[class_indices1]

class_label2 = 0  # Negatif sınıf (temiz sınıf) 0 yapıyoruz
...
true_labels[true_poisoning_indices] = 1  # Pozitif sınıfı 1 yapıyoruz

import matplotlib.pyplot as plt
import numpy as np
import pandas as pd
from sklearn.metrics import roc_curve, auc
true_labels_subset = true_labels[class_indices2]

# Gerçek etiketler ve tahmin edilen skoru roc_curve fonksiyonuna göndererek ROC eğrisini otomatik çizdirebiliriz.
fpr, tpr, thresholds = roc_curve(true_labels_subset, -distances_to_mean)  # -distances_to_mean kullanarak ters sırada hesapla
roc_auc = auc(fpr, tpr)  # AUC hesapla

# ROC eğrisini çiz
plt.figure()
plt.plot(fpr, tpr, color='blue', lw=2, label=f'ROC curve (area = {roc_auc:.2f})')
plt.plot([0, 1], [0, 1], color='gray', linestyle='--')  # Yüksek doğruluk için karşılaştırma çizgisi
plt.xlim([0.0, 1.0])
plt.ylim([0.0, 1.05])
