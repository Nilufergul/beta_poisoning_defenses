...
#tr, val, ts = load_mnist(digits=(9, 7), n_tr=100, n_val=400, n_ts=600)
tr, val, ts = load_data(labels=(6, 8), n_tr=100, n_val=400, n_ts=600)
...
# Mesafeleri küçükten büyüğe sıralama
...
# En büyük farkın olduğu noktayı bul
max_diff_index = np.argmax(distance_diffs)

# En büyük farktan önceki mesafeler küçük mesafeler olarak kabul edilecek
detected_poisoning_indices_sorted = class_indices2[sorted_indices[:max_diff_index + 1]]
...
plt.scatter(X_reduced[detected_poisoning_indices_sorted, 0], X_reduced[detected_poisoning_indices_sorted, 1], 
            c='red', marker='X', label='Sorted Detected Poisoned Points', s=20)

# Ortalama noktayı çiz
plt.scatter(mean_point_reduced[:, 0], mean_point_reduced[:, 1], c='magenta', marker='X', label='Mean Point', s=100)

plt.legend()
plt.title('Mesafeye Göre Sıralama ile Tespit Edilen Zehirlenmiş Noktalar')
plt.xlabel('Component 1')
plt.ylabel('Component 2')
plt.savefig('sorted_accuracy.png')
plt.show()